...
is( $obj->is_member_of('A'),    0, 'not in list A');
is( $obj->get_position('A'),    0, 'no position in list A');
is( $obj->set_position('A', 1), undef, 'no position in list A');
is( $obj->remove_list('A'), undef, 'could not remove from list A');
is( $obj->add_list('A', 1),     1, 'add position 1 to list A');
is( $obj->is_member_of('A'),    1, 'now in list A');
is( int($obj->list_names()),    1, 'member of one list');
is( ($obj->list_names())[0],  'A', 'name of list');
is( $obj->get_position('A'),    1, 'correct position in list A');
is( $obj->set_position('A', 5), 5, 'change position in list A');
...
is( $obj->get_position('A'),    2, 'position in list A');
is( $obj->get_position('B'),    3, 'position in list B');
...
is( $nobj->get_position('A'),    2, 'position in list A');
is( $nobj->get_position('B'),    3, 'position in list B');
